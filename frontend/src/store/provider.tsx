...
export const thunkActions = {
  search: (query, index) => {
    return async function fetchSearch(dispatch, getState) {
      ...
      dispatch(thunkActions.chat(query, index))
    }
  },
  askQuestion: (question, index) => {
    return async function (dispatch, getState) {
      ...
      dispatch(thunkActions.chat(question, index))
    }
  },
  chat: (question: string, index: string) => {
    return async function fetchSearch(dispatch, getState) {
            ...
            if (event.data.startsWith(STREAMING_EVENTS.SESSION_ID)) {
              ...
              try {
                if (source) {
                  ...
                  if (parsedSource.page_content && parsedSource._source.name) {
                    dispatch(
                      actions.addSource({
                        source: {
                          name: parsedSource._source.name,
                          url: parsedSource._source.url,
                          summary: parsedSource.page_content,
                          icon: parsedSource._source.file_extension,
                          updated_at: parsedSource._source.updated_at,
                        },
                      })
                    )
                  }
